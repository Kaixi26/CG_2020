...
void renderScene(void) {
	...
	glBegin(GL_LINES);
	glColor3f(1,1,1);
	glVertex3f(plight[0], plight[1], plight[2]);
	glVertex3f(0, 0, 0);
	glEnd();
...
void processChars(unsigned char c, int xx, int yy) {
	switch(c) {
		case 'q': plight[0]+=0.1; break;
		case 'a': plight[0]-=0.1; break;
		case 'w': plight[1]+=0.1; break;
		case 's': plight[1]-=0.1; break;
		case 'e': plight[2]+=0.1; break;
		case 'd': plight[2]-=0.1; break;
	}
}
